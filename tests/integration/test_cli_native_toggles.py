"""
Tests for the four native-enhancement toggles:
...
(`--native-battle-control` was removed: the native explode/faint controller swap softlocked in
real play and the Variant-3 RAM path is now the single production mechanism.)

Mirrors tests/integration/test_cli_overworld_presence.py.  Unlike the opt-in run RULES
...
def test_help_mentions_all_native_toggles():
    """Sanity: all four flags appear in the --help output, and the removed one does not."""
    res = subprocess.run(
    ...
    assert "--native-battle-control" not in out, "removed flag is back"
...
def test_state_defaults():
    """SoulLinkState defaults: messages/sounds OFF, battle_calc/pc_trade_npc ON."""
    s = SoulLinkState()
...
def test_hello_config_carries_native_toggles(tmp_path, monkeypatch):
    """The hello config command carries all four toggle fields for the Lua client."""
    monkeypatch.setattr("server.state.LINKS_PATH", str(tmp_path / "links.json"))
    state = SoulLinkState()
    ...
    assert "native_battle_control" not in cfg
...
def test_manager_spawn_flags_non_default(tmp_path, monkeypatch):
    ...
    cmd = _captured_spawn_cmd({"run_id": "r2", "tcp_port": 1, "http_port": 2,
                               ...
                               "battle_calc": False, "pc_trade_npc": False,
                               "native_battle_control": True})   # stale key must be ignored
    assert "--native-messages" in cmd
    assert "--native-sounds" in cmd
    assert "--no-battle-calc" in cmd
    assert "--no-pc-trade-npc" in cmd
    # An old registry entry may still carry the removed key; it must not resurrect the flag.
    assert "--native-battle-control" not in cmd
