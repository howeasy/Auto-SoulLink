	...
	$(".set-selector").select2({
		formatResult: function (object, container, query) {
			if ($("#randoms").prop("checked")) {
				...
				function hlText(str) {
					if (!query || !query.term) return str;
					return query.term.split(" ").filter(function (t) { return t; }).reduce(function (s, term) {
						var re = new RegExp("(" + term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + ")", "gi");
						return s.replace(re, '<mark style="background:#e94560;color:#fff;border-radius:2px;padding:0 2px">$1</mark>');
					}, str);
				}
				return object.set ? hlText(object.text) : ("<b>" + hlText(object.text) + "</b>");
			}
		},
		query: function (query) {
			...
			for (var i = 0; i < options.length; i++) {
				...
				if (!query.term || query.term.toUpperCase().split(" ").every(function (term) {
					return pokeName.indexOf(term) >= 0 || setName.indexOf(term) >= 0;
				})) {
					if ($("#randoms").prop("checked")) {
		...
		query: function (query) {
			...
			for (var i = 0; i < options.length; i++) {
				...
				if (option.isCustom && option.set && (!query.term || query.term.toUpperCase().split(" ").every(function (term) {
					return pokeName.indexOf(term) >= 0 || (setName && setName.indexOf(term) >= 0);
				}))) {
					results.push(option);
				}
			}
