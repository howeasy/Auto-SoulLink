...
<body>
  ...
  <script>
    ...
    function refreshUrl(id) { var cfg = document.querySelector('.card-cfg[data-id="'+id+'"]'); if (!cfg) return; var base = cfg.dataset.base; var st = _cardState[id]; var q = []; if (st.params.theme && st.params.theme !== '') q.push('theme=' + st.params.theme); if (st.params.layout && st.params.layout !== '') q.push('layout=' + st.params.layout); if (st.params.speed && st.params.speed !== '1' && st.params.speed !== '') q.push('speed=' + st.params.speed); var filterPills = Array.from(cfg.querySelectorAll('.cpill[data-filter]')); if (filterPills.length > 0) { var activeFilters = filterPills.filter(function(b){ return b.classList.contains('factive'); }); if (activeFilters.length < filterPills.length && activeFilters.length > 0) { q.push('filter=' + activeFilters.map(function(b){ return b.dataset.filter; }).join(',')); } } var url = base + (q.length ? '?' + q.join('&') : ''); var box = document.getElementById(id); if (box) box.textContent = url; var openBtn = document.getElementById(id + '-open'); if (openBtn) openBtn.href = url; var cardEl = cfg.closest('.overlay-card'); if (cardEl) { var ifr = cardEl.querySelector('iframe'); if (ifr) ifr.src = url; } }
